...
/**
 * Replace all instances of @aurodesignsystem, kit, Kit, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\kit(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Kit(?!\()/g, nameExtractionData.nameCap);
